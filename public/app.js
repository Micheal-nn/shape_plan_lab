...
function validateClientInput(input) {
  ...
  if (goal.type !== "maintain" && !hasWeight && !hasBodyFat && !hasCircumferenceTarget) add("goal", "请至少填写目标体重、目标体脂率或目标围度。");
...
function renderPlan(plan) {
  ...
  const calculationsMarkup = logic.calculations.map((item) => `<article class="rationale-item"><h4>${item.labelZh}：${item.value}</h4><p>${item.explanationZh}</p></article>`).join("");
  const evidenceMarkup = logic.evidenceBasis.map((item) => `<article class="rationale-item"><h4>${item.titleZh}</h4><p>${item.textZh}</p></article>`).join("");
